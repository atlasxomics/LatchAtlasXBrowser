...
def generate_spatial(self, qcparams, **kwargs):
    ...
    # source image path
    allFiles = [
        i for i in oldFiles if ".json" not in i and "spatial" not in i.split("/")[:-1]
    ]
    ...
    if not updating_existing:
        ...
        for i in allFiles:
            ...
            if "flow" in i.lower() or "fix" in i.lower():
                try:
                    os.rename(
                        "{}/{}/{}".format(latch_dir, run_id, name),
                        str(figure_dir.joinpath(name)),
                    )
                except:
                    pass
            elif i in bsa_path:
        ...
        self.update_state(
            state="PROGRESS", meta={"position": "running", "progress": 45}
        )

        # generate cropped images using crop parameters
        cropped_bsa = bsa_source.crop(
            (
                crop_coordinates[0],
                crop_coordinates[1],
                crop_coordinates[2],
        ...
        cropped_postB = postB_source.crop(
            (
                crop_coordinates[0],
                crop_coordinates[1],
                crop_coordinates[2],
        ...
        self.update_state(
            state="PROGRESS", meta={"position": "running", "progress": 65}
        )
        if width > height:
            factorHigh = 2000 / width
            factorLow = 600 / width
            high_res = cropped_postB.resize(
                (2000, int(height * factorHigh)), Image.LANCZOS
            )
            low_res = cropped_postB.resize(
                (600, int(height * factorLow)), Image.LANCZOS
            )
        else:
            factorHigh = 2000 / height
            factorLow = 600 / height
            high_res = cropped_postB.resize(
                (int(width * factorHigh), 2000), Image.LANCZOS
            )
        ...
        json.dump(
            scalefactors,
            open(local_scalefactors_filename, "w"),
            indent=4,
            sort_keys=True,
        )
    ...
    for idx, b in enumerate(barcodes):
        ...
        val = 0
        if tixel_positions[keyindex]["value"]:
            val = 1
