...
def generate_spatial(self, qcparams, **kwargs):
    ...
    self.update_state(
        state="PROGRESS", meta={"position": "preparation", "progress": 0}
    )
    ...
    self.update_state(
        state="PROGRESS", meta={"position": "running", "progress": 20}
    )
    barcodes = []
    ...
    json.dump(
        metadata, open(local_metadata_filename, 'w'), indent=4, sort_keys=True
    )
    ...
    # load image from s3
        for i in allFiles:
            ...
            if "flow" in i.lower() or "fix" in i.lower():
                try:
                    os.rename(
                        "{}/{}/{}".format(latch_dir, run_id, name), str(figure_dir.joinpath(name))
                    )
                except:
                    pass
            elif i in bsa_path:
        ...
        cropped_bsa = bsa_source.crop(
            (crop_coordinates[0], crop_coordinates[1], crop_coordinates[2], crop_coordinates[3])
        )
